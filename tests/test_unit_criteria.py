...
def test_datetime_criterion_validation():
    ...
    with pytest.raises(
        TypeError, match="active_start must be a datetime.datetime instance"
    ):
        DateTimeCriterion(
            worker_count=8, active_start=22, active_end=datetime(2026, 1, 1, 3, 0)
        )

    with pytest.raises(
        TypeError, match="active_end must be a datetime.datetime instance"
    ):
        DateTimeCriterion(
            worker_count=8, active_start=datetime(2026, 1, 1, 22, 0), active_end=3
        )
...
def test_time_criterion_validation():
    ...
    with pytest.raises(
        TypeError, match="active_start must be a datetime.time instance"
    ):
    ...
    try:
        ...
        with pytest.raises(ImportError, match="TimeCriterion requires 'pytz' package"):
            criteria_module.TimeCriterion(
                worker_count=8, active_start=time(22, 0), active_end=time(3, 0)
            )
    finally:
        # Restore everything
...
def test_memory_criterion_missing_psutil():
    ...
    try:
        ...
        with pytest.raises(
            ImportError, match="MemoryCriterion requires 'psutil' package"
        ):
...
def test_time_criterion_serialization():
    ...
    criterion = TimeCriterion(
        worker_count=8, active_start=time(22, 0), active_end=time(3, 0), timezone="UTC"
    )
...
def test_multi_criterion_validation():
    time_crit = TimeCriterion(
        worker_count=2, active_start=time(22, 0), active_end=time(3, 0)
    )
    ...
    with pytest.raises(
        TypeError, match="All criteria must be ScalingCriterion instances"
    ):
...
def test_multi_criterion_and_logic(mocker):
    ...
    time_crit = TimeCriterion(
        worker_count=4, active_start=time(22, 0), active_end=time(3, 0)
    )
...
def test_serialization_error_handling():
    ...
    with pytest.raises(KeyError):
        TimeCriterion.from_dict(
            {"worker_count": 8}
        )  # Missing active_start, active_end, timezone
...
def test_conditional_criterion_serialization(mocker):
    ...
    conditional = ConditionalCriterion(
        condition_criterion=time_crit, action_criterion=memory_crit, workers=6
    )
