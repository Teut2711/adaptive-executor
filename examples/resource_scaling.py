...
def main():
    ...
    resource_policy = MultiCriterion(
        criteria=[(cpu_criterion, 4), (memory_criterion, 6)],
        logic="or"
    )
    ...
    print("=" * 40)
    print(f"CPU threshold: {cpu_criterion.threshold}% -> {cpu_criterion.workers} workers")
    print(f"Memory threshold: {memory_criterion.threshold}% -> {memory_criterion.workers} workers")
    ...
    def memory_intensive_task(task_id):
        ...
        _ = sum(data)
        print(f"Memory task {task_id} completed with {executor.current_workers} workers")
